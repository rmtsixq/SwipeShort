...
app.get('/api/stream/:tmdb_id', async (req, res) => {
    try {
        ...
        // vidsrc.xyz embed linki oluştur
        const embedUrl = `https://vidsrc.to/embed/movie/${tmdbID}`;
        console.log("Providing embedUrl to frontend via proxy endpoint:", embedUrl);
        
        // Frontend'in proxy endpoint'ini kullanarak embed URL'ini yüklemesini sağla
        const proxiedEmbedUrl = `/proxy/stream?url=${encodeURIComponent(embedUrl)}`;

        // Frontend'e proxylenmiş embed URL'ini döndür
        res.json({ streamSrc: proxiedEmbedUrl }); // Frontend'in beklediği formatta URL'i döndür

    } catch (err) {
        console.error("Error in /api/stream/:tmdb_id:", err);
